...
import { ArrowLeft, Zap, Play, Terminal, Box, ChevronRight, Loader2 } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="px-3 py-1 rounded-full border border-cyan-500/30 bg-cyan-500/10 text-cyan-400 text-xs font-bold uppercase tracking-wide">
                Instant Access
              </span>
            </div>
          ...
          <div className="space-y-4">
             ...
             <div className="rounded-2xl border border-white/10 bg-slate-900 overflow-hidden">
                <textarea
                  className="w-full h-32 bg-slate-900 p-4 text-gray-300 focus:outline-none resize-none placeholder-gray-600"
                  placeholder="Enter a prompt to test this model..."
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                />
                <div className="px-4 py-3 bg-slate-950/50 border-t border-white/5 flex justify-between items-center">
                  <span className="text-xs text-gray-500">Uses credits from your connected wallet (Simulated)</span>
                  <button 
                    // onClick={handleTestRun}
                    disabled={isLoading || !prompt}
                    className="px-4 py-2 rounded-lg bg-cyan-600 text-white font-medium text-sm hover:bg-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-colors"
                  >
        ...
        <div className="space-y-6">
          ...
          <GlassCard className="p-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-cyan-500/20 to-transparent rounded-full blur-2xl -mr-10 -mt-10" />
            <h3 className="text-lg font-bold text-white mb-2">Endpoint</h3>
            <div className="bg-slate-950 border border-white/10 rounded-lg p-3 flex items-center justify-between group">
              <code className="text-cyan-400 text-sm truncate mr-2">{model.endpoint}</code>
            </div>
